...
namespace Minesweeper___game.Datebase
{
    class Cells
    {
        ...
        public void generateCellsType (int level)
        {
            generateMines(level);
            mineCheck(level);
        }
        public void generateMines(int level)
        {
            int density = 0;
            int mineTotal = 0;
        ...
        public void mineCheck (int level)
        {
            for (int i = 0; i < Game.levels.levelsList[level].height; i++)
            {
                for (int y = 0; y < Game.levels.levelsList[level].width; y++)
                {
                    if (this.board[i, y].type == -1)
                    {
                        neighbourCheck(i - 1, y, level);
                        neighbourCheck(i + 1, y, level);
                        neighbourCheck(i, y - 1, level);
                        neighbourCheck(i, y + 1, level);
                        neighbourCheck(i - 1, y - 1, level);
                        neighbourCheck(i + 1, y - 1, level);
                        neighbourCheck(i - 1, y + 1, level);
                        neighbourCheck(i + 1, y + 1, level);
                    }
        ...
        public void neighbourCheck (int i, int y, int level)
        {
